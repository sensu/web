import { buildFilter, buildFilterParams, parseFilter, parseFilterParams, toggleParam } from "./filterParams";
...
describe("filter param utils", () => {
  ...
  test("parseFilterParams", () => {
    ...
    expect(parseFilterParams(["foo:bar"])).toEqual({foo: "bar"});
    expect(parseFilterParams(["foo:bar", "bar:baz"])).toEqual({foo: "bar", bar: "baz"});
  });
  test("buildFilter", () => {
  ...
  test("toggleParam", () => {
    let x;
    toggleParam("foo", fn => { x = fn({bar:"baz"}) })("bar");
